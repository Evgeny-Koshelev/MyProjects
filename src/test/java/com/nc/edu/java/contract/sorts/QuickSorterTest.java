...
public class QuickSorterTest {
	...
	private Contract[] checkQuickSort = new Contract [5];
	...
	public void testSort() {
		...
		for(int i =0;i<checkQuickSort.length;i++)
		{
			for(int j = 0;j<checkQuickSort[i].getAllFields().length;j++)
			{
				assertEquals(checkQuickSort[i].getAllFields()[j],contracts.getCloneContractsArray()[i].getAllFields()[j]);
			}
...
private void fillListContracts() {
		...
		checkQuickSort[0] = internetOne;
		checkQuickSort[1] = mobileOne;
		checkQuickSort[2] = tvOne;
		checkQuickSort[3] = internetTwo;
		checkQuickSort[4] = tvTwo;
